...
public class funkcjaKwadratowa {
    ...
    private BigDecimal a, b, c, x0, x1, x2;
    ...
    private void obliczEkstremum() {
       ...
       // ekstremum = BigDecimal.ZERO;
    }

    private void rozwiazRownanie(){
        BigDecimal delta = b.pow(2).subtract(a.multiply(c).multiply(BigDecimal.valueOf(4)));
        int porownanie = delta.compareTo(BigDecimal.ZERO);

        if(porownanie == 0) {
            iloscMiejscZerowych = 1;
            x0 = b.negate().divide(a.multiply(BigDecimal.valueOf(4)));
            ...
            x1 = b.negate().add(sqrt).divide(a2);
